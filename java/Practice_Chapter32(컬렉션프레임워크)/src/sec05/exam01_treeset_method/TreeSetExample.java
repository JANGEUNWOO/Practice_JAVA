...
public class TreeSetExample {
	...
	public static void main(String[] args) {
		...
		// 아래와 같이 추가되었을때, 이진트리로 그리면? 어떻게 되나?
		scores.add(new Integer(87)); // scores.add(87);이라고 해도 괜찮다 자동박싱이 일어난다
		...
		// 주어진 객체의 오른쪽에 있는 점수(95점 위의 점수) -- 주어진 객체 미포함
		score = scores.higher(new Integer(95));
		// score = scores.lower(95); //자동박싱
		...
		// 컬렉션에 데이터가 있다면...
		/*
		 * while(!scores.isEmpty()) { //제일 왼쪽 객체부터 가져온다. 단, 가져온 후 treeSet에서 제거가 된다.
		 * score = scores.pollFirst(); System.out.println(score + "(남은 객체 수 : " +
		 * scores.size() + ")"); }
		 */
		// 컬렉션에 데이터가 있다면...
		/*
		 * while(!scores.isEmpty()) { //제일 오른쪽 객체부터 가져온다. 단, 가져온 후 treeset에서 제거가 된다.
		 * score = scores.pollLast(); System.out.println(score + "(남은 객체 수 : " +
		 * scores.size() + ")"); }
		 */
		// 반복자를 얻는다.
		Iterator<Integer> iterator = scores.iterator();
		while (iterator.hasNext()) {
			// pollFirst()와 같은 결과를 가져오긴 하지만 차이점은 객체 제거를 하지 않는것이
			// 가장 큰 차이점이다. 앞에서 말했듯이 명시적으로 remove()를 해줘야 비로소 제거가 됨을 기억하도록 하자.
